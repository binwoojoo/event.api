...
public class EventService {
    ...
    public Map<String, Object> getEvents(int pageNo, String sort, String userId) {
        ...
        List<EventDetailDto> eventDtoList = events.stream().map(EventDetailDto::new).collect(Collectors.toList());
    ...
    public void saveEvent(EventSaveDto dto, String userId) {
        ...
        if (eventUser.getRole() == Role.COMMON && eventUser.getEventList().size() >= 4) {
            throw new IllegalStateException("일반 회원은 더 이상 이벤트를 등록할 수 없습니다.");
        }
